...
if(UNAME)
   ...
   elseif("${processorId}" MATCHES "x86")
      set(chip "x86")
   elseif("${processorId}" MATCHES "aarch64")
      set(chip "aarch64")
   else()
      set(chip "unknown")
   endif()
...
macro(extract_msvc_compiler_info CL)
   if(CL)
      ...
      elseif("${CL}" MATCHES "aarch64")
         set(COMPILER_VERSION "${COMPLILER_VERSION} aarch64")
      else()
...
file(WRITE "${PROJECT_DIRECTORY}/os_ident.txt" "${os_ident}")
list(APPEND CTEST_NOTES_FILES "${PROJECT_DIRECTORY}/os_ident.txt")
...
if(WIN32)
   if("${compiler}" STREQUAL "MSVC")
      ...
      if(NINJA)
         ...
         set(MAKE_ARGS "-j${number_of_processors} -k50")
      elseif(JOM)
...
if(NOT "$ENV{CMAKE_TOOLCHAIN_FILE}" STREQUAL "")
   ...
   list(APPEND configure_options "-DCMAKE_TOOLCHAIN_FILE=${PROJECT_DIRECTORY}/$ENV{CMAKE_TOOLCHAIN_FILE}")
endif()
      ...
      if(res EQUAL 0)
         ...
         if(FALSE AND travis_os) # disable this branch for now, testing subsets on travis
            ...
            if("${subset}" STREQUAL "sse")
               set(label_list other NoSIMD SSE1 SSE2 SSSE3 SSE4_2)
            elseif("${subset}" STREQUAL "sse2")
               set(label_list SSE1 SSE2)
            elseif("${subset}" STREQUAL "ssse3")
               set(label_list SSSE3)
            elseif("${subset}" STREQUAL "sse4")
               set(label_list other NoSIMD SSE4_2)
            elseif("${subset}" STREQUAL "avx")
               set(label_list AVX)
            elseif("${subset}" STREQUAL "avx2")
               set(label_list AVX2)
            elseif("${subset}" STREQUAL "avx512")
               set(label_list AVX512)
            elseif("${subset}" STREQUAL "knl")
               set(label_list KNL)
            elseif("${subset}" STREQUAL "neon")
               set(label_list NEON)
            else()
               set(label_list other NoSIMD SSE1 SSE2 SSE4_2 AVX AVX2 KNL AVX512 NEON)
            endif()
