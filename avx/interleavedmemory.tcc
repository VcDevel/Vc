...
{
    deinterleave(v0, v1, v2, v3);
    const __m128  il0 = _mm_loadl_pi(_mm_setzero_ps(), reinterpret_cast<__m64 const *>(&m_data[m_indexes[0] + I(float_v::Size)])); // a0 b0
    const __m128  il2 = _mm_loadl_pi(_mm_setzero_ps(), reinterpret_cast<__m64 const *>(&m_data[m_indexes[2] + I(float_v::Size)])); // a2 b2
    const __m128  il4 = _mm_loadl_pi(_mm_setzero_ps(), reinterpret_cast<__m64 const *>(&m_data[m_indexes[4] + I(float_v::Size)])); // a4 b4
    const __m128  il6 = _mm_loadl_pi(_mm_setzero_ps(), reinterpret_cast<__m64 const *>(&m_data[m_indexes[6] + I(float_v::Size)])); // a6 b6
    const __m128 il01 = _mm_loadh_pi(             il0, reinterpret_cast<__m64 const *>(&m_data[m_indexes[1] + I(float_v::Size)])); // a0 b0 a1 b1
    const __m128 il23 = _mm_loadh_pi(             il2, reinterpret_cast<__m64 const *>(&m_data[m_indexes[3] + I(float_v::Size)])); // a2 b2 a3 b3
    const __m128 il45 = _mm_loadh_pi(             il4, reinterpret_cast<__m64 const *>(&m_data[m_indexes[5] + I(float_v::Size)])); // a4 b4 a5 b5
    const __m128 il67 = _mm_loadh_pi(             il6, reinterpret_cast<__m64 const *>(&m_data[m_indexes[7] + I(float_v::Size)])); // a6 b6 a7 b7
...
{
    deinterleave(v0, v1, v2, v3);
    const __m128  il0 = _mm_loadu_ps(&m_data[m_indexes[0] + I(float_v::Size)]); // a0 b0 c0 d0
    const __m128  il1 = _mm_loadu_ps(&m_data[m_indexes[1] + I(float_v::Size)]); // a1 b1 c1 d1
    const __m128  il2 = _mm_loadu_ps(&m_data[m_indexes[2] + I(float_v::Size)]); // a2 b2 c2 d2
    const __m128  il3 = _mm_loadu_ps(&m_data[m_indexes[3] + I(float_v::Size)]); // a3 b3 c3 d3
    const __m128  il4 = _mm_loadu_ps(&m_data[m_indexes[4] + I(float_v::Size)]); // a4 b4 c4 d4
    const __m128  il5 = _mm_loadu_ps(&m_data[m_indexes[5] + I(float_v::Size)]); // a5 b5 c5 d5
    const __m128  il6 = _mm_loadu_ps(&m_data[m_indexes[6] + I(float_v::Size)]); // a6 b6 c6 d6
    const __m128  il7 = _mm_loadu_ps(&m_data[m_indexes[7] + I(float_v::Size)]); // a7 b7 c7 d7
