...
namespace Vc
{
    namespace Mem
    {
        template<VecPos L, VecPos H> __m256 ALWAYS_INLINE CONST shuffle128(__m256 x, __m256 y) {
        ...
        template<VecPos L, VecPos H> __m256i ALWAYS_INLINE CONST shuffle128(__m256i x, __m256i y) {
            VC_STATIC_ASSERT(L >= X0 && H >= X0, Incorrect_Range);
            VC_STATIC_ASSERT(L <= Y1 && H <= Y1, Incorrect_Range);
            return _mm256_permute2f128_si256(x, y, (L < Y0 ? L : L - Y0 + 2) + (H < Y0 ? H : H - Y0 + 2) * (1 << 4));
        }
        template<VecPos L, VecPos H> __m256d ALWAYS_INLINE CONST shuffle128(__m256d x, __m256d y) {
            VC_STATIC_ASSERT(L >= X0 && H >= X0, Incorrect_Range);
            VC_STATIC_ASSERT(L <= Y1 && H <= Y1, Incorrect_Range);
        ...
        __m256 ALWAYS_INLINE CONST blend(__m256 x, __m256 y) {
        ...
        template<VecPos Dst0, VecPos Dst1, VecPos Dst2, VecPos Dst3, VecPos Dst4, VecPos Dst5, VecPos Dst6, VecPos Dst7>
        __m256i ALWAYS_INLINE CONST blend(__m256i x, __m256i y) {
        ...
        template<VecPos Dst0, VecPos Dst1, VecPos Dst2, VecPos Dst3, VecPos Dst4, VecPos Dst5, VecPos Dst6, VecPos Dst7>
        __m256 ALWAYS_INLINE CONST permute(__m256 x) {
