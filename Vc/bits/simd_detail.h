#ifndef BITS_SIMD_DETAIL_H_
#define BITS_SIMD_DETAIL_H_
...
#define Vc_WORKAROUND_XXX_3 1

// bad codegen for integer division
#define Vc_WORKAROUND_XXX_4 1
...
inline namespace v2
{
using namespace std; // TODO remove after putting it into std::experimental
...
#if defined __x86_64__ && !defined Vc_HAVE_SSE2
...
#define Vc_NORMAL_MATH [[gnu::__optimize__("finite-math-only,no-signed-zeros")]]
#define Vc_UNREACHABLE __builtin_unreachable
#define Vc_NEVER_INLINE [[gnu::__noinline__]]
#if defined Vc_CLANG
#  define Vc_INTRINSIC [[gnu::__always_inline__]] inline
#else
#  define Vc_INTRINSIC [[gnu::__always_inline__, gnu::__artificial__]] inline
#endif
#define Vc_CONST __attribute__((__const__))
#define Vc_PURE __attribute__((__pure__))
#define Vc_ALWAYS_INLINE [[gnu::__always_inline__]] inline
...
#elif defined __GNUC__
#  define Vc_NODISCARD [[gnu::__warn_unused_result__]]
#else
#  define Vc_NODISCARD
#endif
...
#define Vc_LIST_BINARY(macro) macro(|) macro(&) macro(^)
#define Vc_LIST_SHIFTS(macro) macro(<<) macro(>>)
#define Vc_LIST_ARITHMETICS(macro) macro(+) macro(-) macro(*) macro(/) macro(%)

#define Vc_ALL_BINARY(macro) Vc_LIST_BINARY(macro) Vc_NOTHING_EXPECTING_SEMICOLON
#define Vc_ALL_SHIFTS(macro) Vc_LIST_SHIFTS(macro) Vc_NOTHING_EXPECTING_SEMICOLON
#define Vc_ALL_ARITHMETICS(macro) Vc_LIST_ARITHMETICS(macro) Vc_NOTHING_EXPECTING_SEMICOLON
