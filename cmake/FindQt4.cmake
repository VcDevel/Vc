...
IF (QT4_QMAKE_FOUND)
  ...
  IF (NOT QT_LIBRARY_DIR OR QT_QMAKE_CHANGED)
    ...
    IF(EXISTS "${QT_LIBRARY_DIR_TMP}")
      # support 32 bit builds on 64 bit hosts. If CXXFLAGS contains -m32 then try to use the 32 bit
      # library path
      IF("${CMAKE_CXX_FLAGS}" MATCHES "(^| )-m32($| )")
        IF("${QT_LIBRARY_DIR_TMP}" MATCHES "lib64")
          STRING(REPLACE "lib64" "lib32" QT_LIBRARY_DIR_32 "${QT_LIBRARY_DIR_TMP}")
          IF(EXISTS "${QT_LIBRARY_DIR_32}")
            SET(QT_LIBRARY_DIR_TMP "${QT_LIBRARY_DIR_32}")
          ELSE(EXISTS "${QT_LIBRARY_DIR_32}")
            STRING(REPLACE "lib64" "lib" QT_LIBRARY_DIR_32 "${QT_LIBRARY_DIR_TMP}")
            IF(EXISTS "${QT_LIBRARY_DIR_32}")
              SET(QT_LIBRARY_DIR_TMP "${QT_LIBRARY_DIR_32}")
            ENDIF(EXISTS "${QT_LIBRARY_DIR_32}")
          ENDIF(EXISTS "${QT_LIBRARY_DIR_32}")
        ELSE("${QT_LIBRARY_DIR_TMP}" MATCHES "lib64")
          STRING(REPLACE "lib" "lib32" QT_LIBRARY_DIR_32 "${QT_LIBRARY_DIR_TMP}")
          IF(EXISTS "${QT_LIBRARY_DIR_32}")
            SET(QT_LIBRARY_DIR_TMP "${QT_LIBRARY_DIR_32}")
          ENDIF(EXISTS "${QT_LIBRARY_DIR_32}")
        ENDIF("${QT_LIBRARY_DIR_TMP}" MATCHES "lib64")
