...
macro(OptimizeForArchitecture)
   ...
   if(TARGET_ARCHITECTURE STREQUAL "auto")
      ...
      elseif(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
         exec_program("/usr/sbin/sysctl -n machdep.cpu.features" OUTPUT_VARIABLE _cpuinfo)
         #string(REGEX REPLACE ".*vendor_id[ \t]*:[ \t]+([a-zA-Z0-9_-]+).*" "\\1" _vendor_id "${_cpuinfo}")
         #string(REGEX REPLACE ".*cpu family[ \t]*:[ \t]+([a-zA-Z0-9_-]+).*" "\\1" _cpu_family "${_cpuinfo}")
         #string(REGEX REPLACE ".*model[ \t]*:[ \t]+([a-zA-Z0-9_-]+).*" "\\1" _cpu_model "${_cpuinfo}")
   ...
   foreach(_flag IN LISTS _march_flag_list)
      AddCompilerFlag("-march=${_flag}" _good)
      if(_good)
         break()
      endif(_good)
   endforeach(_flag)
   foreach(_flag IN LISTS _enable_vector_unit_list)
      AddCompilerFlag("-m${_flag}")
   endforeach(_flag)
   foreach(_flag IN LISTS _disable_vector_unit_list)
